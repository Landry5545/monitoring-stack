...
The native systemd node_exporter is still installed and running on `server-vm` (it's a real, working service — just not the one Prometheus actually scrapes), left in place as a reminder of the diagnostic path. `client-vm`'s node_exporter stayed native throughout, since that one was never a Docker-host-loopback problem — it's a separate physical machine in this VM topology, reached over a normal routed network, which is exactly the case Docker bridges handle without issue.

**Takeaway:** when several theoretically-correct fixes fail in exactly the same way, that's usually a sign the layer you're debugging isn't the one that's actually broken. Pivoting to a different architecture (containerize the exporter) cost less time than continuing to chase the routing issue, and is also just a legitimate, common pattern in real Docker monitoring setups — not a workaround to apologize for.
